...
BEGIN
    ...
    -- So for example match 1 and 2 play each other then the winner plays match 3
    -- But if there are more matches, 1,2,3,4 then winner of 1-2 plays winner of 3-4 in match 5
    ...
    v_match_pair_number := (SELECT COUNT(*)
                            ...
                            AND m.winner_id IS NOT NULL) * 2 + 1;
    ...
    AND ((tm.match_id = v_match_pair_number) OR (tm.match_id = v_match_pair_number + 1))
    AND (m.player_1_id IS NULL OR m.player_2_id IS NULL)
    LIMIT 1;

    -- If a match exists, update the existing match
    IF FOUND THEN
        IF m.player_1_id IS NULL THEN
            UPDATE public.match
